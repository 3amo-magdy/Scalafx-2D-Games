...
object MovingTile extends JFXApp3 {
  def WIDTH = 2
  def HEIGHT = 4
  def fxCELLWIDTH = 50
  ...
  def pushL(movingTile:Int,board:Array[Array[Int]]) = {swap(board,tileRow(movingTile),tileCol(movingTile),tileRow(movingTile),tileCol(movingTile)-1);movingTile-1}
  ...
  def pushU(movingTile:Int,board:Array[Array[Int]]) = {swap(board,tileRow(movingTile),tileCol(movingTile),tileRow(movingTile)-1,tileCol(movingTile));movingTile-WIDTH}

  def pushD(movingTile:Int,board:Array[Array[Int]]) = {swap(board,tileRow(movingTile),tileCol(movingTile),tileRow(movingTile)+1,tileCol(movingTile));movingTile+WIDTH}
    ...
    stage = new JFXApp3.PrimaryStage {
      ...
      scene = new Scene(WIDTH*fxCELLWIDTH,HEIGHT*fxCELLWIDTH){
        ...
        onKeyPressed = key=>{
          ...
          draw()
          if(endTest(board)){ new Alert(AlertType.Information, "Well Done !").showAndWait()}
        }
      }
    }
